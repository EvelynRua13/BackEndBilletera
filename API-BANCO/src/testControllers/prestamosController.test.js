...
describe('prestamosController', () => {
    ...
    test('crearSolicitudPrestamo - datos inválidos devuelve 400', async () => {
        ...
        expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ message: 'Datos de solicitud inválidos.' }));
    });
    ...
    test('crearSolicitudPrestamo - usuario con préstamo activo produce error (500) y rollback', async () => {
        ...
        expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ message: 'Error al procesar la solicitud de préstamo.' }));
        // error.message debe contener la razón original
        expect(res.json.mock.calls[0][0].error).toEqual(expect.stringContaining('El usuario ya tiene un préstamo activo'));
    });
    ...
    test('crearSolicitudPrestamo - éxito devuelve 201 con prestamoId', async () => {
        ...
        expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ message: 'Solicitud de préstamo creada exitosamente.', prestamoId: 55 }));
    ...
    test('actualizarEstadoPrestamo - datos faltantes devuelve 400', async () => {
        ...
        expect(res.status).toHaveBeenCalledWith(400);
        expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ message: 'Datos incompletos para actualizar el préstamo.' }));
    });
    ...
    test('actualizarEstadoPrestamo - transición válida: éxito y commit', async () => {
        ...
        const req = { body: { prestamoId: 1, nuevoEstado: 'aceptado', usuarioId: 'ACC123', monto: 1000 } };
        const res = { status: jest.fn().mockReturnThis(), json: jest.fn().mockReturnThis() };
