...
export const actualizarEstadoPrestamo = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({ message: `Préstamo ${nuevoEstado} exitosamente.`, estado: nuevoEstado });
    } catch (error) {
...
export const crearSolicitudPrestamo = async (req, res) => {
    ...
    try {
        ...
        await connection.query(
            ...
            [Number(monto), cuentaId]
        );

        await connection.commit();
        return res.status(201).json({ message: 'Solicitud de préstamo creada exitosamente.', prestamoId: resultado.insertId });
    } catch (error) {
    if (connection) await connection.rollback();
    logger.error('Error al crear la solicitud de préstamo:', error.message);
        return res.status(500).json({ message: 'Error al procesar la solicitud de préstamo.', error: error.message });
    } finally {
        if (connection && typeof connection.release === 'function') connection.release();
    }
};